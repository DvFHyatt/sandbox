...
select
  sa.colleague_id,
  ts.property_id,
  coalesce(ts.department, od.name) as department,
  ts.training_date,
...
left join operational_divisions od on od.id = ts.division_id
join training_types tt on tt.id = ts.training_type_id and tt.deleted_at is null
where sa.is_deleted = false
group by sa.colleague_id, ts.property_id, coalesce(ts.department, od.name), ts.training_date;
...
with rows as (
  ...
  where ts.is_deleted = false
    ...
    and coalesce(ts.department, od.name) = in_department
    and ts.training_date between in_start and in_end
), summary as (
  select
    coalesce(sum(duration_minutes),0) as total_duration_minutes,
    coalesce(count(distinct training_date) filter (where training_type = 'OJT (On the Job Training)'),0) as ojt_days,
...
drop policy if exists "capture sessions by P/G/GA" on training_sessions;
...
drop policy if exists "manage colleagues by P/G/GA" on colleagues;
